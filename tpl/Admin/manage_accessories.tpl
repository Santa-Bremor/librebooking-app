                ...
                <div id="AddAccessoryContent" class="accordion-collapse collapse">
                    <div class="accordion-body add-contents">
                        ...
                        <div class="row gy-2 mb-2">
                            <div class="col-12">
                                <div class="form-group d-flex align-items-center flex-wrap gap-3">
                                    <label class="fw-bold" for="addResponsibleUser">{translate key='AccessoryResponsibleUser'}</label>
                                    <select id="addResponsibleUser" class="form-select w-auto"
                                            {formname key=ACCESSORY_RESPONSIBLE_USER_ID}>
<option value="">— {translate key='NotAssigned'} —</option>
{foreach from=$users item=user}
<option value="{$user->UserId}">{$user->FullName} ({$user->EmailAddress})</option>
                                        {/foreach}
                                    </select>
                                </div>
                            </div>
        ...
        <div class="card-body">
            ...
            <table class="table table-striped table-hover border-top" id="{$tableId}">
                ...
                <tr>
                    ...
                    <th>{translate key='AccessoryResponsibleUser'}</th>
                ...
                {foreach from=$accessories item=accessory}
                    <tr data-accessory-id="{$accessory->Id}"
                        data-responsible-user-id="{$accessory->ResponsibleUserId|default:''}">
                        ...
                        <td>{if $accessory->ResponsibleUserName}{$accessory->ResponsibleUserName}{else}—{/if}</td>
            ...
            <form id="editForm" method="post">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group mb-2">
                            <label class="fw-bold" for="editResponsibleUser">{translate key='AccessoryResponsibleUser'}</label>
                            <select id="editResponsibleUser" class="form-select"
                                    {formname key=ACCESSORY_RESPONSIBLE_USER_ID}>
<option value="">— {translate key='NotAssigned'} —</option>
{foreach from=$users item=user}
<option value="{$user->UserId}">{$user->FullName} ({$user->EmailAddress})</option>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            {foreach from=$accessories item=accessory}
            accessoryManagement.addAccessory('{$accessory->Id}', '{$accessory->Name|escape:javascript}', '{$accessory->QuantityAvailable}', '{$accessory->ResponsibleUserId|default:''}');
            {/foreach}
        });
    </script>
    <script>
